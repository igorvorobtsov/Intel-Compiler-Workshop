...
if [ $# -eq 0 ]; then
    ...
    echo "  1 - Loop multiversioning and vectorization analysis"
...
if [ "$EXERCISE" == "1" ] || [ "$EXERCISE" == "all" ]; then
    ...
    echo -e "• ${GREEN}estimated potential speedup: 6.5x${NC}: Expected performance improvement"
    echo ""
    ...
    echo -e "${BLUE}Estimated speedup:${NC} ~6.5x (limited by double precision)"
    echo ""

    echo -e "${GREEN}=== Creating fixed version with float constant ===${NC}"
...
EOF
    echo -e "${GREEN}Created vec_sin_fixed.cpp with 'f' suffix on constant${NC}"
    echo ""
    ...
    echo -e "  • Estimated speedup: ${RED}~6.5x${NC}"
    echo ""
    ...
    echo -e "  • Estimated speedup: ${GREEN}~13.0x${NC}"
    echo ""
    echo -e "${GREEN}Performance improvement: 2x faster just by adding 'f'!${NC}"
    echo ""
    echo -e "${BLUE}Key lesson:${NC} Optimization reports help identify hidden issues"
    echo -e "like implicit type conversions that hurt performance."
    echo ""
...
if [ "$EXERCISE" == "4" ] || [ "$EXERCISE" == "all" ]; then
    ...
    echo ""
    echo -e "We'll use ${GREEN}the same source file (dist.c)${NC} compiled three ways:"
    echo -e "  ${GREEN}V1:${NC} No pragma (no -DUSE_OMP_SIMD) → inner loop vectorized"
    echo -e "  ${GREEN}V2:${NC} With pragma (-qopenmp -DUSE_OMP_SIMD) → outer loop vectorized"
    echo -e "  ${GREEN}V3:${NC} Pragma + known trip count (-qopenmp -DUSE_OMP_SIMD + -DKNOWN_TRIP_COUNT) → outer loop vectorized, inner loop fully unrolled"
    echo ""

    setup_icx

    echo -e "${GREEN}=== dist.c source (unified for all versions) ===${NC}"
    show_code "dist.c" 1 18
    echo -e "${BLUE}Key: #pragma omp simd is conditionally compiled with -DUSE_OMP_SIMD${NC}"
    echo ""
    ...
    run_cmd "icx -O2 -xHost -qopenmp -DUSE_OMP_SIMD -DKNOWN_TRIP_COUNT -qopt-report=3 -qopt-report-phase=vec -qopt-report-file=stdout dist.c -c"

    echo -e "${GREEN}Result:${NC}"
    echo -e "• ${GREEN}✅ OUTER LOOP WAS VECTORIZED${NC} (pragma active)"
    echo -e "• ${GREEN}✅ Inner loop fully unrolled${NC} (MYDIM=3, only 3 iterations)"
    echo -e "• ${YELLOW}Best performance:${NC} Outer loop vectorized + inner loop unrolled"
    echo ""
    ...
    echo -e "${GREEN}=== Build complete programs ===${NC}"
    echo -e "${BLUE}Same source (dist.c), different compilation flags:${NC}"
    ...
    echo -e "${BLUE}Same source file, three different optimization strategies:${NC}"
    ...
    echo -e "• All versions use SIMD, but in different ways"
    echo -e "• V2 and V3 vectorize outer loop (multiple points simultaneously)"
    echo -e "• V1 vectorizes inner loop (one point at a time)"
    echo ""
...
if [ "$EXERCISE" == "5" ] || [ "$EXERCISE" == "all" ]; then
    ...
    echo -e "${BLUE}========================================${NC}"
    echo -e "• ${GREEN}Vectorization${NC} (V1-AVX vs baseline): 2-4x speedup"
    echo -e "• ${GREEN}Outer loop${NC} strategy (V2, V3 vs V1): Better performance"
    echo -e "• ${GREEN}Inner loop unrolling${NC} (V3 vs V2): Eliminates loop overhead"
    echo -e "• ${GREEN}FMA instructions${NC} (AVX2): Significant boost for multiply-add"
    echo -e "• ${GREEN}512-bit registers${NC} (AVX-512): Can process 16 floats at once"
    echo ""
    echo -e "${YELLOW}Note:${NC} Actual speedup depends on CPU architecture, memory bandwidth,"
    echo -e "and whether AVX-512 causes frequency throttling on your system."
    echo ""
fi
...
if [ "$EXERCISE" == "6" ] || [ "$EXERCISE" == "all" ]; then
    ...
    echo ""
    echo -e "${GREEN}AVX-512 solution:${NC} vcompresspd instruction"
    ...
    echo -e "• ${GREEN}✅ Vector length 8${NC} (8 doubles at once)"
    echo -e "• ${BLUE}Instruction used:${NC} vcompresspd (AVX-512 specific)"
    echo ""

    echo -e "${BLUE}========================================${NC}"
    echo -e "${BLUE}How vcompresspd Works${NC}"
    echo -e "${BLUE}========================================${NC}"
    echo -e "${YELLOW}Step 1: Compare${NC} - Create mask of elements matching condition"
    echo -e "  vcmppd k1, zmm0, zmm1  → k1 = [0,1,0,1] (mask for a[i] > 0)"
    echo ""
    echo -e "${YELLOW}Step 2: Compress${NC} - Store only selected elements contiguously"
    echo -e "  vcompresspd [b]{k1}, zmm0  → Stores only masked values"
    echo ""
    ...
    echo -e "  Input:  [-2.0,  3.0, -1.0,  5.0]"
    echo -e "  Mask:   [   0,    1,    0,    1]"
    echo -e "  Output: [ 3.0,  5.0]  (compressed!)"
    echo ""
    ...
    echo -e "• ${GREEN}AVX-512 provides vcompresspd${NC} specifically for this idiom"
    echo -e "• ${BLUE}Compiler recognizes${NC} the pattern automatically (no pragmas needed)"
    echo -e "• ${YELLOW}Typical speedup:${NC} 2-4x with AVX-512 (depends on selectivity)"
    ...
    echo ""
    echo -e "${BLUE}Selectivity Impact:${NC}"
    echo -e "  • 10% positive → Moderate speedup (~2x)"
    echo -e "  • 50% positive → High speedup (~3-4x)"
    echo -e "  • 90% positive → Maximum speedup (~4-5x)"
    echo ""
fi
